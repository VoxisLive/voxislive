...
Used by the Python audio engine and the webui bridge:
    ...
    * App manifest  → GET  /app.json     (public, unauthenticated; version + langs)
...
def fetch_app_manifest() -> dict | None:
    ...
    Deliberately NOT gated behind IS_OFFICIAL_RELEASE like the rest of this
    module: it is an unauthenticated GET carrying no user data (no JWT, no
    device id), so it does not touch the billing/telemetry layer the OSS/BYOK
    build promises to stay off of. Both builds benefit — an update-available
    hint and language-coverage info with zero privacy cost.

    Fails silently (None) on any network/parse error; the caller must never
    let a missing manifest block or degrade a translation session."""
    try:
        resp = _http.get(f"{_BASE_URL}/app.json", timeout=_TIMEOUT)
        if resp.status_code != 200:
            return None
        return resp.json()
    except (requests.RequestException, ValueError) as exc:
        _log_detail("fetch_app_manifest", exc)
        return None
