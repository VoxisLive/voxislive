...
def windows_edition() -> str:
    """"10" / "11" / "" (non-Windows or detection failed), derived from the
    build number (>= 22000 is Win11) rather than platform.release() — Windows
    11 still self-reports as release "10" on some Python builds since the two
    share a release() family, so the build number is the reliable signal."""
